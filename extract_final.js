...
const data = {
    ...
    name_components: {
		...
		nouns_plural: extractArray(/var ln_fm="([^"]+)"/, ',')
    },
    levels: {
		quality: extractArray(/function tl\([^]*?var ln="([^"]+)"/, ';'),
		size: extractArray(/function tavern_size\([^]*?var nl1="([^"]+)"/, '|')
	},
    prices: [],
    meals: [],
    drinks: []
...
for (const match of priceMatches) {
	if (match[1] !== "---") {
		// Clean up HTML tags and map to an array of lines
		data.prices.push(match[1].replace(/<br>/g, '\n').trim());
	}
}

// Save to JSON files
fs.writeFileSync('location.json', JSON.stringify(data.location, null, 2));
fs.writeFileSync('name.json', JSON.stringify(data.name_components, null, 2));
fs.writeFileSync('level.json', JSON.stringify(data.levels, null, 2));
fs.writeFileSync('price.json', JSON.stringify([...new Set(data.prices)], null, 2));
fs.writeFileSync('meal.json', JSON.stringify([...new Set(data.meals)], null, 2));
fs.writeFileSync('drink.json', JSON.stringify([...new Set(data.drinks)], null, 2));

console.log("Extraction complete. Final files created.");
